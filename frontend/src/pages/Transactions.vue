...
const openStockSelectModal = async () => {
    ...
    // 异步拉取最新
    try {
        const res = await fetch('http://localhost:3000/stocks/popular?n=5')
...
function handleSearch(val) {
    ...
    searchTimeout = setTimeout(async () => {
        ...
        errorMsg.value = ''
        try {
            const res = await fetch('http://localhost:3000/stocks/search?query=' + encodeURIComponent(val))
            const data = await res.json()
            if (Array.isArray(data) && data.length > 0) {
                stockOptions.value = data
            } else {
...
async function handleAddStocks(stocks) {
    for (const stock of stocks) {
        if (!watchlist.find(item => item.symbol === stock.symbol)) {
            ...
            try {
                const res = await fetch('http://localhost:3000/watchlist/add', {
                    method: 'POST',
...
async function handleAddShare(item, index) {
    ...
    try {
        const res = await fetch(`http://localhost:3000/stocks/price/${item.symbol}`)
        const data = await res.json()
...
onMounted(async () => {
    try {
        const res = await fetch('http://localhost:3000/watchlist/100023')
        const data = await res.json()
