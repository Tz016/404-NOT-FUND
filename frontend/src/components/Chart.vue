...
const renderChart = () => {
    ...
    const items = props.assets
        ...
        .map(a => {
            const shares = clean(a.shares || a.quantities);
            const lastPrice = clean(a.last_price);
            const value = shares * lastPrice;
            const cost = clean(a.total_cost);
            return {
                name: a.symbol,
                cost,
                value,
                change: value - cost // 手动计算盈亏
            };
        });
    ...
    labels.each(function (d) {
        ...
        const datum = items.find(x => x.name === d.data.name);
        const percent = totalAssetsValue > 0 ? datum.value / totalAssetsValue : 0;
        const textContent = `${datum?.name || d.data.name} (${d3.format(".1%")(percent)})`;
